...
interface SegyErrorStateProps {
  filename?: string
  error: string
}

export function SegyErrorState({ filename, error }: SegyErrorStateProps) {
  return (
      ...
      <Card className="border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-950/30">
        <CardHeader>
          <CardTitle className="text-red-800 dark:text-red-200 flex items-center space-x-2">
            ...
            <span>Error Loading SEGY File</span>
          </CardTitle>
          <CardDescription className="text-red-600 dark:text-red-400">
            {filename ? `Failed to load ${filename}` : 'Failed to load SEGY file'}
          </CardDescription>
        </CardHeader>
        <CardContent>
